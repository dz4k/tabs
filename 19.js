...
const makelogger = (scope) =>
  (...args) => (console.log("%c%s", "color:green", scope, ...args), args.length === 1 ? args[0] : args);
...
export
const
$ = (selOrScope, sel) => (sel ? selOrScope : document).querySelector(sel ? sel : selOrScope),
$$ = (selOrScope, sel) => Array.from((sel ? selOrScope : document).querySelectorAll(sel ? sel : selOrScope)),
...
att = (el, name, ...args) => args.length > 0 ? el.setAttribute(name, args[0]) : el.getAttribute(name),
next = traverse("next"),
